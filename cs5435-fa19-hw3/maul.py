...
def do_attack():
  ...
  #you'll need to change this to a non-admin user, such as 'victim'.
	...
	print (admin_cookie)
	#string representing hex first 2 bytes represent the data

	admin_cookie=admin_cookie[0]+hex(int(admin_cookie[1],16)^1)[2:]+admin_cookie[2:]
	print (admin_cookie)
	sess.cookies.set('admin',None)
	sess.cookies.set('admin',admin_cookie)
